...
def prepare_for_cuda_graph(device, nbytes: int = 131072) -> torch.Tensor:
    ...
    if key not in _TMA_WORKSPACES:
        ...
        # capture hits the cache instead of allocating inside the pool:
        #   hadamard_amax_triton.py:180  — positional: (sv, A.device, bfloat16, dim)
        #   hadamard_quantize_row_col_triton.py:303 — keyword: (sign_vector=, device=, hadamard_dimension=)
        # Use torch.device(key) ("cuda:N") because A.device always produces a fully-indexed
        # device and lru_cache keys are compared by value.
...
# ---------------------------------------------------------------------------
# FP4 packing — round-to-nearest and stochastic-rounding variants
# ---------------------------------------------------------------------------
...
@triton.jit
def _nvfp4_quantize(a_t_rht, global_amax, BLOCK_N: tl.constexpr, BLOCK_M: tl.constexpr):
...
def _swizzle_scales(scale_inv, BLOCK_OUTER: tl.constexpr, BLOCK_INNER: tl.constexpr):
    ...
    """
    scale_inv = tl.reshape(scale_inv, [BLOCK_OUTER // 128, 4, 32, BLOCK_INNER // 64, 4])
